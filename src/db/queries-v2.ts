...
export function getWatchLater(options: {
  ...
  const rows = db.prepare(`
    SELECT wl.id, wl.priority, wl.remind_at, wl.note, wl.tags,
    ...
    ${where}
    ORDER BY CASE wl.priority WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END,
      wl.remind_at ASC NULLS LAST, wl.added_at ASC
  `).all(...params) as any[];
...
export function updateWatchLaterStatus(id: number, status: 'done' | 'skipped' | 'pending'): void {
  getDb().prepare(`
    UPDATE watch_later SET status = ?,
...
export function updateWatchLaterPriority(id: number, priority: 'high' | 'medium' | 'low'): void {
  getDb().prepare('UPDATE watch_later SET priority = ? WHERE id = ?').run(priority, id);
}

export function updateWatchLaterNote(id: number, note: string): void {
  getDb().prepare('UPDATE watch_later SET note = ? WHERE id = ?').run(note, id);
}
...
export function saveComments(videoYoutubeId: string, comments: any[]): void {
  ...
  const insertMany = db.transaction((items: any[]) => {
    for (const c of items) {
      stmt.run(
        video.id, c.youtube_comment_id, c.author_name,
        c.author_channel_id || null, c.is_channel_owner ? 1 : 0,
        c.text, c.like_count || 0, c.reply_count || 0,
        c.published_at || null, c.parent_id || null
      );
    }
  });
...
export function getCachedComments(videoYoutubeId: string, options: {
  limit?: number; ownerOnly?: boolean; withReplies?: boolean;
} = {}): CommentRow[] {
  const db = getDb();
  const { limit = 20, ownerOnly = false, withReplies = false } = options;

  const where = ownerOnly ? "AND c.is_channel_owner = 1" : "AND c.parent_id IS NULL";

  const rows = db.prepare(`
    SELECT c.* FROM comments c JOIN videos v ON v.id = c.video_id
    WHERE v.youtube_id = ? ${where}
    ORDER BY c.is_channel_owner DESC, c.like_count DESC LIMIT ?
  `).all(videoYoutubeId, limit) as any[];

  let result: CommentRow[] = rows.map(r => ({
    ...r, is_channel_owner: !!r.is_channel_owner, replies: [],
  }));
  ...
  if (withReplies && result.length > 0) {
    ...
    result = result.map(r => ({
      ...r, replies: repliesMap.get(r.youtube_comment_id) || [],
    }));
  }
